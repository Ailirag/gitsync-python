...
Зачем это здесь: без явного ключа `-ReportFormat` `/ConfigurationRepositoryReport` пишет
табличный документ, и расширение файла на это не влияет — в файл с именем `.txt` всё равно
попадает MOXCEL. Проверено на живом конфигураторе 8.3.27.2130 (см.
`tests/fixtures/native/PROVENANCE.md`).

Уточнение: текстовый формат у платформы есть — документация 8.3.27 описывает
`-ReportFormat <txt|mxl>` со значением по умолчанию `mxl`. Продукт этот ключ не передаёт
и разбирает MOXCEL; так проверено на стенде, и менять рабочий разбор ради txt здесь не
требуется.
